...
def natural_language_to_pbs_filters(preferences_text, trip_data=None):
    ...
    weekend_phrases = [
        ...
        'saturdays off', 'sundays off', 'sat off', 'sun off', 'family time',
        'weekend home'
    ]
    ...
    # ==========================================
    early_phrases = [
        'no early', 'avoid early', 'late start', 'no morning', 'avoid morning',
        'no early departure', 'no early departures', 'sleep in',
        'not a morning person', 'no early shows', 'no early report',
        'late show'
    ]
    ...
    # Short trips
    short_phrases = [
        'short trip', 'day trip', '1 day', 'one day', 'short', 'quick turn',
        'turn', 'day trips', 'single day', 'home every night',
        'home each night', 'no overnight', 'home daily', 'local'
    ]
    if any(phrase in text_lower for phrase in short_phrases):
        filters.append("PREFER TRIPS WITH DUTY_DAYS <= 2")
        if 'home' in text_lower and ('night' in text_lower
    ...
    redeye_phrases = [
        ...
        'avoid red-eye', 'no overnight', 'no late night', 'no all-nighter',
        'avoid night flying'
    ]
    ...
    # ==========================================
    commute_phrases = [
        'commut', 'late start', 'commutable', 'easy commute', 'drive in',
        'live far', 'non-local', 'late show'
    ]
    ...
    credit_phrases = [
        ...
        'maximum pay', 'high pay', 'credit time', 'build time', 'high time',
        'max hours', 'overtime', 'premium pay', 'productive', 'efficiency'
    ]
    ...
    # ==========================================
    intl_phrases = [
        'international', 'europe', 'asia', 'london', 'paris', 'frankfurt',
        'tokyo', 'overseas', 'long haul', 'widebody', 'atlantic', 'pacific',
        'LHR', 'CDG', 'FRA', 'NRT'
    ]
    ...
    for day_name, day_code in days.items():
        if day_name in text_lower:
            if 'no ' + day_name in text_lower or 'avoid ' + day_name in text_lower:
                filters.append(
                    f"AVOID TRIPS IF DUTY_PERIOD INCLUDES {day_code}")
    ...
    # ==========================================
    reserve_phrases = [
        'no reserve', 'avoid reserve', 'no standby', 'line holder'
    ]
    ...
    # ==========================================
    holiday_phrases = [
        'christmas', 'thanksgiving', 'new year', 'july 4', 'memorial day',
        'labor day', 'holiday'
    ]
    ...
    # ==========================================
    qol_phrases = [
        'quality of life', 'work life balance', 'family first', 'lifestyle',
        'easy month', 'relaxed'
    ]
    ...
    return unique_filters if unique_filters else [
...
def test_enhanced_nlp():
    ...
    test_cases = [
        "I want weekends off", "No early morning departures",
