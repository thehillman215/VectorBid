...
def get_user_bid_system(user_id: str) -> BidLayersSystem:
    ...
    if user_id not in user_bid_systems:
        ...
        system.add_layer(create_layover_preference_layer(["LAX", "SFO", "DEN"], 8))
    ...
    try:
        ...
        for filter_data in data["filters"]:
            try:
                ...
                if criteria in [
                    FilterCriteria.TRIP_LENGTH,
                    FilterCriteria.CREDIT_HOURS,
                ]:
                    try:
                        value = float(value)
                    ...
                    FilterCriteria.WEEKENDS_OFF,
                    FilterCriteria.RED_EYE,
                    FilterCriteria.INTERNATIONAL,
                    FilterCriteria.COMMUTABLE,
                ]:
...
def format_recommendation(recommendation):
    formats = {
        "HIGHLY_RECOMMENDED": {"text": "Excellent Match", "class": "bg-success"},
        "RECOMMENDED": {"text": "Good Match", "class": "bg-primary"},
