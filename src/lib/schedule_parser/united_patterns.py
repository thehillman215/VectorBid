...
def parse_united_content(text_content: str, filename: str = "") -> List[Dict]:
    ...
    for line_num, line in enumerate(lines):
        ...
        # Skip header lines
        if any(header in line.upper() for header in ['TRIP', 'PAIRING', 'CREDIT', 'ROUTING', 'DAYS']):
        ...
        for pattern_info in patterns:
            ...
            if match:
                ...
                if parsed_trip:
                    logger.debug(f"Line {line_num + 1}: Matched {pattern_name} - {parsed_trip['trip_id']}")
                    break
...
def _extract_united_trip(match: re.Match, line: str, confidence: float, 
                        pattern_name: str, line_num: int) -> Optional[Dict]:
    ...
    try:
        ...
        if 'credit' in groups and groups['credit']:
            credit_str = groups['credit'].replace(':', '.').replace('H', '').replace('CR', '').strip()
            try:
        ...
        logger.warning(f"Failed to extract United trip from line {line_num}: {line[:50]}... Error: {e}")
        return None
...
def _detect_united_weekend(line: str, dates: str = "", routing: str = "") -> bool:
    ...
    weekend_patterns = [
        r'\bSAT\b', r'\bSUN\b',           # SAT, SUN
        r'\bSATURDAY\b', r'\bSUNDAY\b',   # Full day names
        r'\bWEEKEND\b',                   # Direct mention
        r'\bSA\b', r'\bSU\b',             # Two-letter abbreviations
        r'SAT[A-Z]', r'SUN[A-Z]',         # SAT/SUN followed by letter (like SATA, SUNB)
    ]
...
def parse_with_united_patterns(text_content: str, filename: str = "") -> List[Dict]:
    ...
    # Log results
    if trips:
        logger.info(f"Parsed {len(trips)} {airline} trips with avg confidence {sum(t.get('confidence', 0) for t in trips) / len(trips):.2f}")
    else:
