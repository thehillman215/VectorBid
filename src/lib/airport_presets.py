...
def get_pilot_persona_airport_weights(persona):
    ...
    presets = {
        'credit_hunter': {
            ...
            'LHR': 35, 'FRA': 35, 'CDG': 30, 'AMS': 30,
            'NRT': 40, 'ICN': 40, 'HKG': 35, 'SIN': 35,
            'MUC': 25, 'ZUR': 25, 'BOM': 30, 'DEL': 30
        },
        'family_friendly': {
            # Shorter time zones, easier adjustments
            'LHR': 30, 'FRA': 25, 'CDG': 25, 'AMS': 20,
            'NRT': 15, 'ICN': 15, 'HKG': 10, 'SIN': 10,
            'MUC': 20, 'ZUR': 20, 'BOM': 5, 'DEL': 5
        },
        'adventure_seeker': {
            # Exotic destinations, interesting layovers
            'LHR': 20, 'FRA': 20, 'CDG': 25, 'AMS': 25,
            'NRT': 40, 'ICN': 35, 'HKG': 40, 'SIN': 40,
            'MUC': 30, 'ZUR': 30, 'BOM': 45, 'DEL': 40
        },
        'commuter': {
            # Shorter flights, easier connections
            'LHR': 40, 'FRA': 35, 'CDG': 30, 'AMS': 35,
            'NRT': 10, 'ICN': 10, 'HKG': 15, 'SIN': 15,
            'MUC': 25, 'ZUR': 20, 'BOM': 5, 'DEL': 5
        },
        'work_life_balance': {
            # Balanced approach, reasonable destinations
            'LHR': 25, 'FRA': 25, 'CDG': 20, 'AMS': 20,
            'NRT': 20, 'ICN': 20, 'HKG': 18, 'SIN': 18,
            'MUC': 15, 'ZUR': 15, 'BOM': 12, 'DEL': 12
        }
    }
    
    return presets.get(persona, presets['work_life_balance'])

def enhanced_international_scoring_with_persona(trip, preferences_lower, pilot_persona='work_life_balance'):
