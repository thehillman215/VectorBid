    ...
    def evaluate_trip(self, trip: dict[str, Any]) -> tuple[bool, list[str], float]:
        ...
        if layer_matches:
            total_weight = sum(
                ...
                if i < len(filter_results) and filter_results[i]
            )
            ...
            "layers_detail": [
                {
                    "layer_number": layer.layer_number,
                    "name": layer.name,
                    "filters_count": len(layer.filters),
...
def create_weekends_off_layer(priority: int = 10) -> BidLayer:
    weekend_filter = BidFilter(
        criteria=FilterCriteria.WEEKENDS_OFF,
...
def create_layover_preference_layer(cities: list[str], priority: int = 7) -> BidLayer:
    ...
    for city in cities:
        filter_obj = BidFilter(
            criteria=FilterCriteria.LAYOVER_CITY,
