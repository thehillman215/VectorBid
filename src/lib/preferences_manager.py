...
class PreferencesManager:
    ...
    def update_preferences(self, user_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for key in ['time_preferences', 'route_preferences', 'aircraft_preferences', 
                       'layover_preferences', 'credit_preferences']:
                if key in updates:
    ...
    def learn_from_selections(self, user_id: str, selected_trips: List[Dict], 
                            rejected_trips: List[Dict]) -> Dict[str, Any]:
    ...
    def _calculate_optimization_score(self, profile: Dict[str, Any]) -> float:
        ...
        if profile.get('airline'): score += 10
        if profile.get('base'): score += 10 
        if profile.get('seat'): score += 10
        if profile.get('seniority'): score += 10
        
        # Preference completeness (40 points)
        if profile.get('persona') or profile.get('custom_preferences'): score += 20
        if profile.get('fleet'): score += 10
        if profile.get('learned_patterns'): score += 10
        
        # Activity and learning (20 points)
        if profile.get('preference_history'): score += 10
        if profile.get('last_preferences_update'): score += 10
    ...
    def _calculate_completeness(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'overall_percentage': int(((required_complete + min(preference_complete, 1)) / (len(required_fields) + 1)) * 100)
        }
