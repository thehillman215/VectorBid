...
def enhanced_international_scoring(trip, preferences_lower):
    ...
    european_secondary = {
        'MAD': ('Madrid', 12), 'BCN': ('Barcelona', 10), 'FCO': ('Rome', 15),
        'MXP': ('Milan', 12), 'VIE': ('Vienna', 12), 'BRU': ('Brussels', 12),
        'ARN': ('Stockholm', 10), 'CPH': ('Copenhagen', 10), 'OSL': ('Oslo', 10),
        'LIS': ('Lisbon', 10), 'DUB': ('Dublin', 8)
    }
    ...
    if found_destinations:  # Only boost if actually international
        ...
        if 'european' in preferences_lower or 'europe' in preferences_lower:
            europe_codes = ['LHR', 'FRA', 'CDG', 'AMS', 'MUC', 'ZUR', 'MAD', 'FCO', 'VIE', 'BRU', 'LIS', 'DUB']
