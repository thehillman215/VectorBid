...
def parse_pilot_preferences(preferences_text, pilot_profile=None):
    ...
    prefs["no_redeyes"] = any(
        phrase in text_lower
        for phrase in ["no redeye", "avoid redeye", "no red-eye", "no overnight flying"]
    )
    prefs["max_days_off"] = any(
        phrase in text_lower
...
def create_good_layers_20(prefs):
    ...
    for i in range(6, 11):
        if i == 6:
            filters = [
                "AWARD IF TRIP.CREDIT_HOURS >= 4.0 PER DAY",
                "AWARD IF TRIP.LEGAL_REST_COMPLIANCE = TRUE",
            ]
            if prefs["weekends_off"]:
            ...
            filters = [
                "AWARD IF TRIP.CREDIT_HOURS >= 4.8 PER DAY",
                "AWARD IF TRIP.DUTY_PERIOD <= 13_HOURS",
            ]
            ...
            filters = [
                f"AWARD IF TRIP.STARTS_AT_BASE = {prefs['base']}",
                "AWARD IF TRIP.DEADHEAD_LEGS <= 1",
            ]
            ...
            filters = [
                "AWARD IF TRIP.CREDIT_HOURS >= 4.2 PER DAY",
                "AWARD IF TRIP.LAYOVER_QUALITY >= ADEQUATE",
            ]
            ...
            filters = [
                "AWARD IF TRIP.CREDIT_HOURS >= 4.5 PER DAY",
                "AVOID IF TRIP.DUTY_PERIOD > 14_HOURS",
            ]
...
def create_acceptable_layers_20(prefs):
    ...
    for i in range(11, 16):
        if i == 11:
            ...
            filters = [
                "AWARD IF TRIP.TOTAL_DUTY_TIME <= 100_HOURS",
                "AVOID IF TRIP.CONSECUTIVE_DUTY_DAYS > 6",
            ]
            ...
            filters = [
                ...
                "AWARD IF TRIP.LEGAL_COMPLIANCE = TRUE",
            ]
...
def create_fallback_layers_20(prefs):
    ...
    fallback_configs = [
        (
            16,
            "Any reasonable quality trip",
            "Quality with minimal restrictions",
            "Very High (75-85%)",
            [
                "AWARD IF TRIP.REASONABLE_QUALITY = TRUE",
                "AVOID IF TRIP.DUTY_PERIOD > 15_HOURS",
            ],
        ),
        (
            17,
            "Legal compliance with safety standards",
            "Safety and legal minimums",
            "Very High (80-90%)",
            [
                "AWARD IF TRIP.FAR_117_COMPLIANT = TRUE",
                "AVOID IF TRIP.DUTY_PERIOD > 16_HOURS",
            ],
        ),
        (
            18,
            "Any trip on qualified equipment",
        ...
        (
            19,
            "Any contractually legal assignment",
        ...
        (
            20,
            "Final safety net - any legal assignment",
