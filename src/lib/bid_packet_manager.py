...
class BidPacketManager:
                ...
                for filename in os.listdir(self.bid_storage_path):
                    if filename.endswith('.pdf'):
                        ...
                        packet = {
                            ...
                            'year': int(month_tag[:4]) if len(month_tag) >= 4 else datetime.now().year,
                            'month': int(month_tag[4:6]) if len(month_tag) >= 6 else datetime.now().month,
    ...
    def upload_bid_packet(self, file, month_tag: str, airline: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save metadata
            metadata_file = os.path.join(self.metadata_path, f"{month_tag}_{airline}.json")
            with open(metadata_file, 'w') as f:
    ...
    def delete_bid_packet(self, month_tag: str, airline: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'success': True,
                'message': f'Bid packet deleted: {filename}'
            }
    ...
    def get_bid_packet_file(self, month_tag: str, airline: str) -> Optional[Tuple[bytes, str]]:
