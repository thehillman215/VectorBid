...
def detect_pilot_persona_from_preferences(preferences):
    ...
    elif any(word in prefs_lower for word in ['adventure', 'variety', 'interesting', 'exotic', 'new']):
        return 'adventure_seeker'
    elif any(word in prefs_lower for word in ['commute', 'commuter', 'short', 'quick', 'efficient']):
        return 'commuter'
    else:
