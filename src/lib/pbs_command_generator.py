...
class RateLimiter:
    ...
    def allow_request(self) -> bool:
        ...
        self.requests = [
            r for r in self.requests if r > now - self.window_seconds
        ]
...
class CommandCache:
    ...
    def get(self, preferences: str,
            profile: PilotProfile) -> Optional[GenerationResult]:
    ...
    def set(self, preferences: str, profile: PilotProfile,
...
class LLMGenerator:
    ...
    def generate(self, preferences: str,
                 profile: PilotProfile) -> Optional[list[PBSCommand]]:
...
5. Are specific to this pilot's situation"""
    ...
    def _parse_llm_response(self, response: dict) -> list[PBSCommand]:
        ...
        for cmd_data in response.get('commands', []):
            ...
            command = PBSCommand(command=cmd_data.get('command', ''),
...
class PatternMatcher:
    ...
    PATTERNS = {
        'weekends': [
            (['weekends off', 'no weekends', 'avoid weekends'
             ...
             'Keep weekends free', 1, CommandCategory.WEEKENDS),
            (['saturday off',
              'no saturday'], 'AVOID TRIPS IF DUTY_PERIOD OVERLAPS SAT',
             'Keep Saturdays free', 1, CommandCategory.WEEKENDS),
            (['sunday off',
              'no sunday'], 'AVOID TRIPS IF DUTY_PERIOD OVERLAPS SUN',
             'Keep Sundays free', 1, CommandCategory.WEEKENDS),
        ],
        'time_of_day': [
            (['no early', 'avoid early',
              'late start'], 'AVOID TRIPS IF DEPARTURE_TIME < 0800',
             'Avoid early morning departures', 2, CommandCategory.TIME_OF_DAY),
            (['early departure', 'morning flight'
              ], 'PREFER TRIPS IF DEPARTURE_TIME BETWEEN 0600 AND 1000',
             'Prefer morning departures', 3, CommandCategory.TIME_OF_DAY),
            (['no late', 'avoid late'], 'AVOID TRIPS IF ARRIVAL_TIME > 2200',
             'Avoid late arrivals', 3, CommandCategory.TIME_OF_DAY),
            (['no redeye', 'avoid redeye', 'no red-eye'
              ], 'AVOID TRIPS IF DEPARTURE_TIME BETWEEN 2200 AND 0559',
             'Avoid red-eye flights', 2, CommandCategory.TIME_OF_DAY),
        ],
        'trip_length': [
            (['short trip', 'day trip', '1 day',
              'turns'], 'PREFER TRIPS IF DUTY_DAYS <= 2',
             'Prefer shorter trips', 4, CommandCategory.TRIP_LENGTH),
            (['long trip', '4 day',
              'four day'], 'PREFER TRIPS IF DUTY_DAYS >= 4',
             'Prefer longer trips', 4, CommandCategory.TRIP_LENGTH),
        ],
        'pay': [
            (['high pay', 'max pay', 'maximize pay',
              'high credit'], 'SORT BY CREDIT_HOURS DESCENDING',
             'Maximize pay', 2, CommandCategory.PAY),
            (['efficient', 'productive'], 'PREFER TRIPS IF CREDIT_RATIO > 1.2',
             'Prefer efficient trips', 3, CommandCategory.PAY),
        ],
        'international': [
            (['international', 'overseas', 'europe',
              'asia'], 'PREFER TRIPS IF INTERNATIONAL = TRUE',
             'Prefer international flights', 3, CommandCategory.LOCATIONS),
            (['domestic only',
              'no international'], 'AVOID TRIPS IF INTERNATIONAL = TRUE',
             'Avoid international flights', 3, CommandCategory.LOCATIONS),
        ],
        'commute': [
            (['commute', 'commutable',
              'drive to'], 'PREFER TRIPS IF DEPARTURE_TIME >= 1000',
             'Allow commute time', 2, CommandCategory.COMMUTE),
        ],
        'holidays': [
            (['christmas',
              'xmas'], 'AVOID TRIPS IF DATE_RANGE INCLUDES 24DEC-26DEC',
             'Christmas off', 1, CommandCategory.SPECIFIC_DATES),
            (['thanksgiving'
              ], 'AVOID TRIPS IF DATE_RANGE INCLUDES 4TH_THU_NOV',
             'Thanksgiving off', 1, CommandCategory.SPECIFIC_DATES),
            (['new year'], 'AVOID TRIPS IF DATE_RANGE INCLUDES 31DEC-01JAN',
             'New Year off', 1, CommandCategory.SPECIFIC_DATES),
        ]
    }

    def generate(self, preferences: str,
                 profile: PilotProfile) -> list[PBSCommand]:
        ...
        if not commands:
            commands.append(
                PBSCommand(command='PREFER TRIPS IF QUALITY_OF_LIFE = HIGH',
    ...
    def _generate_profile_commands(self,
                                   profile: PilotProfile) -> list[PBSCommand]:
        ...
        if profile.base:
            ...
            if match:
                code = match.group(1)
                commands.append(
                    PBSCommand(command=f'PREFER TRIPS IF BASE = {code}',
        ...
        if profile.commuter:
            commands.append(
                PBSCommand(command='PREFER TRIPS IF DUTY_DAYS >= 3',
...
class PBSCommandGenerator:
    ...
    def __init__(self):
        ...
        self.cache = CommandCache(Config.MAX_CACHE_SIZE,
                                  Config.CACHE_TTL_SECONDS)
        logger.info("PBS Command Generator initialized")

    def generate(self,
                 preferences_text: str,
                 pilot_profile: Optional[dict] = None) -> dict:
        """
        ...
        # Build result
        result = self._build_result(commands, preferences_text,
                                    generation_method,
    ...
    def _build_result(self, commands: list[PBSCommand], preferences_text: str,
                      method: str, time_ms: int) -> GenerationResult:
        ...
        return GenerationResult(commands=commands,
                                formatted=formatted,
                                stats=stats,
                                errors=[],
                                warnings=warnings,
    ...
    def _format_commands(self, commands: list[PBSCommand],
                         preferences_text: str, method: str) -> str:
    ...
    def _calculate_stats(self, commands: list[PBSCommand],
                         method: str) -> dict:
        ...
        avg_confidence = sum(
            cmd.confidence
            for cmd in commands) / len(commands) if commands else 0
    ...
    def _detect_conflicts(self, commands: list[PBSCommand]) -> bool:
        ...
        has_weekend_avoid = any('SAT OR SUN' in cmd.command for cmd in commands
                                if cmd.category == CommandCategory.WEEKENDS)
        has_long_trips = any('DUTY_DAYS >= 4' in cmd.command
                             for cmd in commands)
        ...
        has_no_early = any('DEPARTURE_TIME < ' in cmd.command
                           for cmd in commands)
    ...
    def _check_warnings(self, commands: list[PBSCommand]) -> list[str]:
        ...
        if high_priority_count > 4:
            warnings.append(
                "Many high-priority items may be difficult to satisfy")
...
def generate_pbs_commands(preferences_text: str,
                          pilot_profile: Optional[dict] = None) -> dict:
    """
...
if __name__ == "__main__":
    ...
    test_cases = [{
        "preferences":
        "I want weekends off and no early morning departures. Prefer high-credit trips.",
        "profile": {
            "base": "Denver (DEN)",
            "seniority": 65,
            "fleet": ["737", "757"]
        ...
        "preferences":
        "Maximize pay, I'll fly anything including redeyes. International preferred.",
        "profile": {
        ...
        "preferences":
        "Christmas and Thanksgiving off, avoid weekends when possible",
        "profile": {
            "base": "Chicago (ORD)",
            "seniority": 70,
            "fleet": ["320", "321"]
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        print(
            f"  Categories: {', '.join(result['stats'].get('category_list', []))}"
        )
