...
class EnhancedPBSGenerator:
            ...
            if re.search(pattern, text):
                self.commands.append(
                    PBSCommand(
                        ...
                        explanation=
                        "Prefers trips that include weekend work for higher pay",
                        layer_number=15))
                break
    ...
    def _process_time_preferences(self, text: str):
        ...
        early_patterns = [
            ...
            r'not before (\d{1,2}):?(\d{2})?', r'after (\d{1,2}):?(\d{2})?'
        ]
                ...
                if 'not before' in match.group() or 'after' in match.group():
                    ...
                    self.commands.append(
                        PBSCommand(
                            ...
                            priority=Priority.HIGH,
                            explanation=
                            f"Prefers trips starting after {time_val[:2]}:{time_val[2:]} for better commute",
                            layer_number=3))
                else:
                    self.commands.append(
                        PBSCommand(
    ...
    def _process_trip_length_preferences(self, text: str):
        ...
        for pattern in short_trip_patterns:
            if re.search(pattern, text):
                self.commands.append(
                    PBSCommand(command_type=CommandType.PREFER,
                               subject="TRIPS",
                               condition="WITH DUTY_DAYS",
                               value="<= 2",
                               priority=Priority.MEDIUM,
                               explanation=
                               "Prefers shorter trips for more time at home",
                               layer_number=10))
                break
            ...
            if re.search(pattern, text):
                self.commands.append(
                    PBSCommand(
                        ...
                        priority=Priority.MEDIUM,
                        explanation=
                        "Prefers longer trips for efficiency and higher credit",
                        layer_number=12))
                break
    ...
    def _process_destination_preferences(self, text: str):
        ...
        for pattern in intl_avoid_patterns:
            if re.search(pattern, text):
                self.commands.append(
                    PBSCommand(command_type=CommandType.AVOID,
                               subject="TRIPS",
            ...
            if re.search(pattern, text):
                self.commands.append(
                    PBSCommand(
                        ...
                        priority=Priority.MEDIUM,
                        explanation=
                        "Prefers international routes for variety and pay premiums",
                        layer_number=13))
                break
                ...
                if 'avoid' in text or 'no' in text:
                    self.commands.append(
                        PBSCommand(
                            command_type=CommandType.AVOID,
                            subject="TRIPS",
                            condition="IF AIRCRAFT IN" if isinstance(
                                types, list) else "IF AIRCRAFT =",
                            value=value,
                            priority=Priority.MEDIUM,
            ...
            if re.search(pattern, text):
                ...
                self.commands.append(
                    PBSCommand(
                        ...
                        priority=Priority.HIGH,
                        explanation=
                        "Prefers late starts on weekdays for commuter accessibility",
                        layer_number=6))
                ...
                self.commands.append(
                    PBSCommand(
                        ...
                        priority=Priority.HIGH,
                        explanation=
                        "Prefers trips ending early enough for same-day commute home",
                        layer_number=7))
                break
            ...
            if re.search(pattern, text):
                self.commands.append(
                    PBSCommand(
                        ...
                        priority=Priority.MEDIUM,
                        explanation=
                        "Prefers high-credit trips for maximum pay efficiency",
                        layer_number=14))
                break
    ...
    def _process_specific_dates(self, text: str):
        ...
        date_patterns = [
            r'home for .*(\d{1,2})(st|nd|rd|th)?', r'need (\w+) (\d{1,2})',
            r'important on (\d{1,2})', r'birthday.*(\d{1,2})',
            r'anniversary.*(\d{1,2})'
        ]
                ...
                try:
                    ...
                    self.commands.append(
                        PBSCommand(
                            ...
                            priority=Priority.CRITICAL,
                            explanation=
                            f"Must be home on the {date_num}th for personal commitment",
                            layer_number=1))
                except:
            ...
            if re.search(pattern, text):
                self.commands.append(
                    PBSCommand(
                        ...
                        priority=Priority.MEDIUM,
                        explanation=
                        "Optimizes schedule for maximum days off and family time",
                        layer_number=11))
                break
    ...
    def _assign_layers(self):
        ...
        self.commands.sort(
            key=lambda cmd: (cmd.priority.value, 0
                             if cmd.command_type == CommandType.AVOID else 1
                             if cmd.command_type == CommandType.PREFER else 2))
    ...
    def generate_pbs_output(self, include_explanations: bool = True) -> str:
        ...
        output_lines = [
            "VectorBid Generated PBS Commands", "=" * 40,
    ...
    def validate_commands(self) -> List[str]:
        ...
        for cmd in self.commands:
            ...
            if not cmd.subject or not cmd.condition or not cmd.value:
                errors.append(
                    f"Layer {cmd.layer_number}: Incomplete command structure")
        return errors
