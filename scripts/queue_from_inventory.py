...
if not inv.exists():
    print("merge_inventory.csv not found"); sys.exit(1)
...
def guess_category(branch: str) -> str:
    b = branch.lower()
    if b.startswith(("fix/", "bugfix/", "hotfix/")): return "bugfix"
    if b.startswith(("infra/", "devops/", "ci/", "build/")): return "infra"
    if b.startswith(("depfu/", "dependabot/", "renovate/")): return "dep-bump"
    if b.startswith(("chore/", "docs/")): return "chore"
    if "experiment" in b or "spike" in b or b.startswith("exp/"): return "experiment"
    if b.startswith(("feat/", "feature/")): return "feature"
    return "feature"
...
def strategy(category:str, conflicts:str, ahead:int) -> str:
    if category=="bugfix": return "squash"
    if category=="infra": return "rebase" if conflicts=="no" else "merge-commit"
...
def decision(category:str, risk:str, conflicts:str) -> str:
    if category=="experiment": return "HOLD"
    if conflicts=="yes" and risk=="high": return "HOLD"
    return "MERGE"
...
with inv.open() as f:
    ...
    for r in rdr:
        ...
        notes=[]
        if conflicts=="yes": notes.append("rebase on release/2025-08-17 first")
        if cat=="feature": notes.append("gate behind flag if incomplete")
        rows.append({
            "branch": b,
            "ahead_of_main": ahead,
            "behind_main": behind,
...
# order queue
cat_order = {"bugfix":0,"infra":1,"chore":2,"docs":2,"feature":3,"dep-bump":4,"experiment":5}
