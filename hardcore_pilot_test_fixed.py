...
def test_nlp(description, input_text, min_commands=1, must_contain=None):
    ...
    print(f"\n{status} [{description}]")
    print(
        f"   Input: '{input_text[:60]}{'...' if len(input_text) > 60 else ''}'"
    )
...
test_cases = [
    ("Slam-clicker lifestyle",
     "I'm a slam-clicker, want good hotels and long layovers, no hub turns", 3,
     ["LAYOVER", "HOTEL", "HUB_TURN"]),
    ("High-time turns", "Need high-time turns for max pay", 3,
     ["DUTY_DAYS", "CREDIT"]),
    ("CDO avoidance", "No CDOs or standups, too old for that", 1,
     ["CONTINUOUS_DUTY"]),
    ("Deadheading", "Avoid deadheading, I want to fly not ride", 1,
     ["DEADHEAD"]),
    ("Min rest", "No min rest layovers, need proper sleep", 1,
     ["MINIMUM_REST"]),
...
test_cases = [
    ...
    ("Conflicting preferences",
     "Max credit but also want to be home every night and weekends off", 4),
...
test_cases = [
    ("West coast commuter",
     "Commuting from LAX to DEN, need late shows and early releases", 3,
     ["AFTER", "BEFORE"]),
    ("East coast commuter",
     "Live in Boston but based in Newark, need commutable on both ends", 2,
     ["COMMUT"]),
    ("Two-day commute", "I commute so prefer 4-day trips to minimize commutes",
     2, ["DUTY_DAYS = 4"]),
]
...
test_cases = [
    ("Soccer mom schedule",
     ...
     2, ["TUE", "THU"]),
    ("Weekend warrior",
     "I run marathons on weekends, absolutely need Sat Sun off", 1,
     ["SAT", "SUN"]),
    ("Night owl",
...
test_cases = [
    ...
    ("Anniversary planning",
     "Wedding anniversary June 20-22 need those days off", 1),
    ("School schedule", "Kids spring break March 10-17 want to be off", 1),
    ("Training month",
     "Have recurrent training this month need 3 days available", 1,
     ["TRAINING"]),
]
...
test_cases = [
    ...
    ("Widebody international",
     "Want widebody international flying, preferably 777 to Asia", 3,
     ["777", "INTERNATIONAL", "ASIA"]),
    ("Domestic narrow", "Staying domestic on the 737, no international", 2,
     ["737", "DOMESTIC"]),
...
test_cases = [
    ("Guarantee plus",
     "Looking to fly guarantee plus 15-20 hours for extra pay", 1, ["CREDIT"]),
    ("Reserve improvement",
     "If I get reserve at least make it long call not short call", 1,
     ["LONG_CALL"]),
    ("Vacation touch",
     "Have vacation days 15-20, prefer trips that touch those days for extra time off",
     1, ["VACATION"]),
    ("Training credit",
     "Doing captain upgrade need sim time built into schedule", 1,
     ["TRAINING"]),
]
...
test_cases = [
    ("Weather phobia",
     "Avoid midwest in winter and Florida in summer due to weather", 1),
    ("Airport specific", "Hate LAX and ORD, avoid at all costs", 2,
     ["LAX", "ORD"]),
    ("Medical needs",
     "Need consistent schedule for medical treatments every Monday morning", 1,
     ["MON"]),
    ("Timezone preference", "Prefer staying in same timezone to avoid jet lag",
     1),
]
...
test_cases = [
    ...
    ("Middle seniority balance",
     ...
     1),
    ("Bottom feeder",
     "Dead last in seniority, just don't want reserve every month", 2,
     ["JUNIOR", "RESERVE"]),
]
...
if percentage >= 90:
    ...
    print(
        f"{RED}{BOLD}❌ NEEDS WORK{RESET} - Critical patterns are being missed."
    )

# Category breakdown
print(f"\n{BOLD}Performance by Category:{RESET}")
categories = [
    "Pilot Slang", "Complex Multi", "Commuter", "Lifestyle", "Dates/Times",
    "Equipment", "Union Contract", "Edge Cases", "Seniority", "Real Messages"
]
