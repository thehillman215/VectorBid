...
_UNITED_PATTERNS = [
    ...
    re.compile(
        r'(?P<id>H\d{4}).*?'
        r'DAYS-\s*(?P<days>\d+).*?'
        r'CRD-\s*(?P<credit>\d{1,2}\.\d{2})', re.DOTALL),

    # Fallback for any H-prefixed 4-digit IDs with credit hours
    re.compile(r'(?P<id>H\d{4}).*?'
               r'(?P<credit>\d{1,2}\.\d{2})', re.DOTALL)
]
...
_STANDARD_PATTERNS = [
    ...
    re.compile(
        ...
        r"Credit:\s*(?P<credit>\d{1,2}\.\d{2})", re.IGNORECASE)
]
...
def _extract_united_routing(context: str) -> str:
    ...
    airports = set([
        'IAH', 'DEN', 'EWR', 'SFO', 'LAX', 'ORD', 'LAS', 'LGA', 'BNA', 'FLL',
        'CUN', 'DFW', 'ATL', 'PHX', 'SEA', 'BOS'
    ])
...
def _deduplicate_and_enhance_trips(trips: list[dict],
