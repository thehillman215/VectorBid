...
def parse_csv(blob: bytes) -> list[dict]:
    ...
    try:
        ...
        for row in reader:
            ...
            trip_id = _get_field(row, [
                'TripID', 'Trip', 'ID', 'trip_id', 'trip', 'id', 'Trip ID',
                'Pairing'
            ])
            days = _get_field(row, [
                'Days', 'days', 'Duration', 'duration', 'Length', 'Trip Days'
            ])
            credit = _get_field(row, [
                'Credit', 'credit', 'Hours', 'hours', 'Credit Hours',
                'Flight Hours'
            ])
            ...
            if trip_id:
                try:
                    trip = {
                        ...
                        str(trip_id).strip(),
                        "days":
                        int(float(days)) if days else 1,
                        "credit_hours":
                        float(credit) if credit else 0.0,
                        "includes_weekend":
                        _detect_weekend_csv(row),
                        "routing":
                        _get_field(
                            row,
                            ['Routing', 'routing', 'Route', 'route', 'Cities'])
                        or '',
                        "raw":
                        dict(row),
                    }
