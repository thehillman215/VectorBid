...
def _pick(endpoint: str) -> str:
    ...
    raise AssertionError(
        f"Could not find path for {endpoint}. Have: {sorted(_OPENAPI_PATHS)[:10]}..."
    )
...
def test_full_pipeline_golden(client, scenario):
    ...
    r = client.post(
        _pick("strategy"),
        json={"preferences": parsed, "optimized": optimized, "context": ctx},
    )
    assert r.status_code == 200, r.text
    strategy = r.json()
