...
def compat_validate(payload: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    try:
        violations = validate_feasibility(pairings, DEFAULT_RULES, context) or []
    except Exception:
        # Last-resort: run without explicit rules; fallback enforces rest>=10h
        violations = validate_feasibility(pairings, None, context) or []
