...
SCORING_CATEGORIES: Tuple[str, ...] = (
    "layovers","award_rate","text_relevance","contract_compliance","fatigue_risk",
    "seniority_bonus","historic_success","recency","diversity",
)
__all__=["rank_candidates","select_topk","SCORING_CATEGORIES","_get_scoring_weights"]

# --- Optional v1/v2 backends (kept for lower-level tests) ---
try:    from .optimizer_v1 import rank_candidates as _rank_v1
except Exception: _rank_v1=None
try:    from .optimizer_v2 import rank_candidates as _rank_v2
except Exception: _rank_v2=None
...
def _tokenize(s: str) -> List[str]: return [t.lower() for t in _WORD_RE.findall((s or ""))]
...
def _get_scoring_weights(
    ...
    overrides: Optional[Mapping[str,float]]=None,*,normalize: bool=True
)->Dict[str,float]:
    base: Dict[str,float]
    ...
    if is_bundle:
        ...
        if not isinstance(dw,Mapping): dw={}
        # start with context defaults
        base={}
        for k,v in dict(dw).items():
            if isinstance(v,(int,float)) and v>0: base[str(k)]=float(v)
...
def _normalize_v2_to_v1(items: Sequence[Mapping[str, Any]])->List[Dict[str,Any]]:
    ...
    for it in items:
        rid=_coerce_id(it)
        try: score=float(it.get("score",0))
        except Exception: score=0.0
        m=dict(it); m["id"]=rid; m["score"]=score; out.append(m)
    out.sort(key=lambda x:(-x["score"],x["id"]))
    return out

def _fallback_v1_rank(candidates: Sequence[Mapping[str,Any]],query: Union[str,Any],*,top_k:int=10,strategy:str="bm25",**kwargs:Any)->List[Dict[str,Any]]:
    ...
    for it in candidates:
        rid=_coerce_id(it); toks=_tokenize(_extract_text_blob(it))
        score=0.0 if not toks else sum(q_weight.get(t,1.0) for t in (set(q_tokens)&set(toks)))/(1.0+0.1*len(toks))
...
def rank_candidates(candidates: Sequence[Mapping[str,Any]],query: Union[str,Any],*,top_k:int=10,strategy:str="bm25",**kwargs:Any)->List[Dict[str,Any]]:
    use_v2=os.getenv("OPTIMIZER_V2","0")=="1" and callable(_rank_v2)
    if use_v2:
        out=_rank_v2(candidates=candidates,query=str(query),top_k=top_k,strategy=strategy,**kwargs)
        return _normalize_v2_to_v1(out)[:top_k]
    if callable(_rank_v1):
        out=_rank_v1(candidates=candidates,query=str(query),top_k=top_k,strategy=strategy,**kwargs)
        norm=[]
        for it in out:
            rid=_coerce_id(it)
            try: sc=float(it.get("score",0))
            except Exception: sc=0.0
            m=dict(it); m["id"]=rid; m["score"]=sc; norm.append(m)
...
def _extract_bundle(bundle: Any)->Dict[str,Any]:
    ...
    if pairings is None: pairings=_get_in(bundle,"pairing_features","pairings",default=[])
    if not isinstance(pairings,Sequence): pairings=[]
    pairings=[_as_dict(p) if not isinstance(p,Mapping) else dict(p) for p in pairings]
    return {"context":_as_dict(context),"prefs":_as_dict(prefs),"analytics":_as_dict(analytics),"compliance":_as_dict(compliance),"pairings":pairings}
...
def _score_pairing(
    ...
    prefer=_get_in(prefs,"soft_prefs","layovers","prefer",default=[]) or _get_in(prefs,"layovers","prefer",default=[])
    lay_base=0.5 if (not prefer or not city) else (1.0 if (isinstance(prefer,Sequence) and city in prefer) else 0.0)
    bd["layovers"]=W.get("layovers",0.0)*lay_base
    if lay_base==1.0: notes.append(f"+layovers({city})")
    ...
    if rate_missing: rate=0.5
    bd["award_rate"]=W.get("award_rate",0.0)*float(rate)

    # Fatigue penalty
    fatigue_w=W.get("fatigue_risk",0.0); pen_raw=0.0
    if fatigue_w:
        if bool(p.get("redeye",False)): pen_raw+=1.0
        rest=p.get("rest_hours")
        if isinstance(rest,(int,float)) and rest<10: pen_raw+=0.5
        if pen_raw:
    ...
    if isinstance(sp,(int,float)):
        sp_eff = min(float(sp), 0.2) if rate_missing else float(sp)  # slight dampening when analytics missing
        bd["seniority_bonus"]=0.1*sp_eff*base

    score = base + bd["seniority_bonus"]
    return score, notes, bd

def select_topk(bundle: Any, top_k:int=10,*,strategy:str="bm25",**kwargs:Any)->List[RankedCandidate]:
    parts=_extract_bundle(bundle); ctx, prefs, analytics, pairings = parts["context"],parts["prefs"],parts["analytics"],parts["pairings"]
    W=_get_scoring_weights(bundle,normalize=True)
    ranked=[]
    for p in pairings:
        pid=_coerce_id(p)
        s,notes,bd=_score_pairing(p,ctx,prefs,analytics,W)
        ranked.append(RankedCandidate(candidate_id=pid,score=float(s),rationale="; ".join(notes) if notes else "baseline",soft_breakdown=bd,pairings=[pid]))
