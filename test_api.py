...
def test_admin_upload_auth(base_url):
    ...
    # Test without auth
    try:
        response = requests.post(f"{base_url}/admin/upload-bid")
    ...
    try:
        ...
        response = requests.post(f"{base_url}/admin/upload-bid", headers=headers)
        if response.status_code == 401:
...
def test_invalid_endpoints(base_url):
    ...
    try:
        response = requests.get(f"{base_url}/admin/upload-bid")
